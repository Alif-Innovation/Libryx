              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors group">
                  ...
                  <td className="px-4 py-3.5 hidden md:table-cell">
                    <span className="text-xs font-mono text-zinc-500">{item.serialNumber}</span>
                  </td>
                  <td className="px-4 py-3.5 hidden lg:table-cell">
                    <span className="text-xs text-zinc-600">{item.category}</span>
                  </td>
                  <td className="px-4 py-3.5 hidden sm:table-cell">
                    <Badge label={item.condition} size="xs" />
                  </td>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Input
                label="Item Name *" id="name" placeholder="e.g. iPad Pro 12.9&quot;"
                value={form.name} onChange={(e) => field('name', e.target.value)}
                error={errors.name}
              />
            </div>
            <Input
              label="Brand *" id="brand" placeholder="e.g. Apple"
              value={form.brand} onChange={(e) => field('brand', e.target.value)}
              error={errors.brand}
            />
            <Input
              label="Model" id="model" placeholder="e.g. M2 (2022)"
              value={form.model} onChange={(e) => field('model', e.target.value)}
            />
            <Input
              label="Serial Number *" id="serialNumber" placeholder="e.g. DLXK9-4421"
              value={form.serialNumber} onChange={(e) => field('serialNumber', e.target.value)}
            ...
            <Select
              ...
              value={form.condition} onChange={(e) => field('condition', e.target.value)}
            >
